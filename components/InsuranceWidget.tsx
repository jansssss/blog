...
// 기본 스타일 (밝은 파스텔 테마)
const DEFAULT_GRADIENT = 'from-emerald-50 via-teal-50 to-cyan-50'
...
export default function InsuranceWidget({
  ...
  const [mobileSlide, setMobileSlide] = useState(0) // 0: 날씨+건강보험, 1: 자동차보험

  // 보험 정보 (2026년 기준 정적 데이터)
  const insuranceData = {
    healthRate: 7.09, // 건강보험료율 (%)
    carAverage: 82, // 자동차보험 평균 (만원/년)
  }
          ...
          >
            ...
            <div className="flex items-center justify-between gap-2 min-w-full px-1">
              ...
              <div className="flex items-center gap-2 backdrop-blur-sm bg-white/60 px-3 py-1.5 rounded-full shadow-sm">
                ...
                <div className="h-3 w-px bg-gray-300" />
                <span className="text-sm font-bold text-gray-900">{insuranceData.healthRate}%</span>
              </div>
            </div>

            {/* 슬라이드 2: 자동차보험 */}
            <div className="flex items-center justify-center gap-2 min-w-full px-1">
              {/* 자동차보험 */}
              <div className="flex items-center gap-2 backdrop-blur-sm bg-white/60 px-3 py-1.5 rounded-full shadow-sm">
                <div className="flex items-center gap-1 text-xs font-medium text-gray-600">
                  <Car className="w-3 h-3" />
                ...
                <div className="h-3 w-px bg-gray-300" />
                <span className="text-sm font-bold text-gray-900">{insuranceData.carAverage}만원/년</span>
              </div>
            </div>
          </div>
