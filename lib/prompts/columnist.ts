...
[자동 실패 조건]
아래 중 하나라도 미충족 시 오류로 간주:
- 금지 표현 사용 (이 글을 통해, 알아보겠습니다, 살펴봅니다 등)
- 모호한 마무리 표현 (생각해보세요, 점검해보세요 등)
- "해당되는 사람 / 덜 해당되는 사람" 구분 2회 미만
- "❌ 안 되는 사람(제외 기준)" 또는 "✅ 안 될 때 대안" 섹션 누락
- 1인칭 시점("저는/제가") 3회 미만 사용
- 감정 표현("놀랐다/걱정됐다/기뻤다" 등) 없음

[경고 조건] (실패는 아니지만 개선 필요):
- 도입부(1500자 내)에 질문(물음표) 없음
- 구체적 숫자(날짜, 금액, 기간 등) 5개 미만 (cleanDraft에 숫자가 적은 경우 허용)
- 숫자 뒤 판단 해석 문장 부족
- 전문성 판단 프레임 부족 (핵심은/정리하면/결국/기준/신호/해당/제외 등)`
